...
.controller('CardCtrl', function($rootScope, $scope, $state, $stateParams, $ionicHistory, $ionicModal, $ionicPopup, $timeout, $translate, $http, Cart, Survey, Profile, Points) {
...
.controller('CartSendCtrl', function($scope, $state, $stateParams, $ionicHistory, $ionicPopup, $timeout, $ionicLoading, $translate, Profile, Cart, Order, Inst) {
  ...
  $scope.$on('$ionicView.enter', function() {
    ...
    $translate(['conn_err_tit', 'conn_err_tmp', 'cart_cntr_pop_tit', 'conn_err_close', 'cart_cntr_pop_tmp', 'cart_cntr_pop_send_ord_tit', 'cart_cntr_pop_send_ord_tmp', 'cart_cntr_pop_send_ord_btn1', 'cart_cntr_pop_send_ord_btn2', 'cart_cntr_pop_send_ord_send', 'cart_cntr_pop_err1_tit', 'cart_cntr_pop_err1_tmp', 'cart_cntr_pop_err2_tit', 'cart_cntr_pop_err2_tmp', 'cart_cntr_pop_nottake_ord_tit', 'cart_cntr_pop_nottake_ord_tmp', 'cart_cntr_pop_nottake_ord_btn1', 'loading_cancel']).then(function(res) {
        ...
        $scope.loading_cancel = res.loading_cancel;
    });
        ...
        buttons: [
          {
            ...
            onTap: function(e) {
              ...
              $timeout(function() {
                $ionicLoading.show({
                  template: '<ion-spinner icon="android" class="spinner-positive"></ion-spinner> '+$scope.cart_cntr_pop_send_ord_send+'...<br/><button class="button" ng-click="loadingHide()">'+$scope.loading_cancel+'</button>',
                  duration: 0
                }).then(function(){
...
.controller('ContactMapCtrl', function($rootScope, $cordovaDevice, $scope, $state, $stateParams, $ionicHistory, $ionicModal, $cordovaGeolocation, $timeout, $translate, Inst, Cart) {
  ...
  function addPlacemark(inst) {
    ...
    placemarkx.events.add('click', function () {
      ...
      var modaltemp = '<ion-modal-view class="modalviewmap"><ion-header-bar><h1 class="title"></h1><button class="button button-clear button-small" ng-click="modal.remove()" style="color:#006C30;"><i class="icon ion-close"></i></button></ion-header-bar><ion-content><div class="row"><div class="col col-75"><h3 class="hh3"><strong>'+inst.office_adress+'</strong></h3>'+indicat+'<span>'+worktime+opncls+'</span></div><div class="col" style="text-align:right;"><img src="img/signs/navigate.png" height="15" /><small style="color:#006C30 !important;">'+$scope.distanceTo+' м</small></div></div><div class="row" style="border-top:1px solid rgba(0,0,0,0.2);"><div class="col" style="vertical-align:middle;" onclick="window.open(\'tel:'+formatPhone(inst.office_tel[0])+'\', \'_system\', \'location=yes\')"><h3 class="hh3" style="display:inline-block;margin:10px;">'+formatPhone(inst.office_tel[0])+'</h3><img src="img/btn/btncall.png" style="height:40px;float:right;" /></div></div><div class="row" style="border-top:1px solid rgba(0,0,0,0.2);"><div class="col" style="vertical-align:middle;" onclick="window.open(\'http://website.com\', \'_system\')"><h3 class="hh3" style="display:inline-block;margin:10px;">'+inst.office_site+'</h3><img src="img/btn/btnwww.png" style="height:40px;float:right;" /></div></div></ion-content></ion-modal-view>';
  ...
  function getLocation() {
    ...
    permissions.hasPermission(permissions.ACCESS_COARSE_LOCATION, function(persucc) {
...
.controller('ElseCtrl', function($rootScope, $scope, $state, $stateParams, $ionicHistory, $ionicModal, $ionicPopup, $ionicActionSheet, $cordovaSQLite, $cordovaDevice, $timeout, $translate, $ionicLoading, $http, $cordovaNetwork, $cordovaCamera, $cordovaFile, $cordovaFileTransfer, $cordovaImagePicker, $cordovaBarcodeScanner, Inst, Cart, Bill, Profile, HtmlEnt, FAQ) {
  ...
  $scope.reviewSavePic = function(imgpath, randpicname) {
      ...
      // Setup the loader
      $ionicLoading.show({
          content: '<ion-spinner icon="android" class="spinner-positive"></ion-spinner> '+$scope.loadingpercent+'</span><br/><button class="button" ng-click="loadingHide()">'+$scope.loading_cancel+'</button>',
          animation: 'fade-in',
          showBackdrop: true,
          maxWidth: 50,
          showDelay: 0
...
.controller('EntryCtrl', function($rootScope, $scope, $state, $stateParams, $timeout, $ionicScrollDelegate, $http, $ionicLoading, $ionicHistory, $cordovaSQLite, $ionicNavBarDelegate, $ionicPopup, $ionicPlatform, $translate, Entry, HtmlEnt, Profile) {
  ...
  $scope.sendOrder = function() {
      ...
      if(orderstr.length > 0) {
        ...
        $ionicLoading.show({
            content: '<i class="icon ion-loading-c"></i><br/><button class="button" ng-click="loadingHide()">'+$scope.loading_cancel+'</button>',
            animation: 'fade-in',
            showBackdrop: false,
            maxWidth: 50,
            showDelay: 0
...
.controller('ProfileEditCtrl', function($rootScope, $scope, $state, $stateParams, $ionicHistory, $ionicActionSheet, $ionicModal, $ionicPopup, $ionicLoading, $http, $cordovaSQLite, $cordovaDevice, $timeout, $cordovaNetwork, $cordovaCamera, $cordovaFile, $cordovaFileTransfer, $cordovaImagePicker, $translate, $ionicPickerI18n, ionicDatePicker, Profile, Phone, HtmlEnt, timezoneAdd, timezoneSub) {
  ...
  $scope.goback = function() {
    Profile.allnew().then(function(data) {
      ...
      ($scope.profile.user_birthday == '0000-00-00') ? $scope.userbirthday = $scope.newdate.getTime() : $scope.userbirthday = new Date($scope.profile.user_birthday).getTime();
      $ionicHistory.goBack();
    }, function() {});
  }
  ...
  $scope.$on('$ionicView.enter', function() {
    ...
    $translate(['conn_err_tit', 'conn_err_tmp', 'conn_err_close', 'profile_edit_ctrl_pop_proofed_tit', 'profile_edit_ctrl_pop_proofed_temp', 'profile_edit_ctrl_pop_caution_tit', 'profile_edit_ctrl_pop_caution_tmp', 'profile_edit_ctrl_pop_5minagain_tit', 'profile_edit_ctrl_pop_5minagain_tmp', 'profile_edit_ctrl_pop_numagain_tit', 'profile_edit_ctrl_pop_numagain_tmp', 'profile_edit_ctrl_pop_codeagain_tit', 'profile_edit_ctrl_pop_codeagain_tmp', 'profile_edit_ctrl_pop_reqerr_tit', 'profile_edit_ctrl_pop_reqerr_tmp', 'profile_edit_ctrl_pop_answerr_tit', 'profile_edit_ctrl_pop_answerr_tmp', 'profile_edit_ctrl_pop_discount_tit', 'profile_edit_ctrl_pop_discount_tmp', 'profile_edit_ctrl_pop_block_tit', 'profile_edit_ctrl_pop_block_tmp', 'profile_edit_ctrl_pop_deblock_tit', 'profile_edit_ctrl_pop_deblock_tmp', 'profile_edit_ctrl_pop_numconf_tit', 'profile_edit_ctrl_pop_numconf_tmp', 'profile_edit_ctrl_pop_numconf_btn1', 'profile_edit_ctrl_pop_erransw_tit', 'profile_edit_ctrl_pop_erransw_tmp', 'profile_edit_ctrl_pop_errcode_tit', 'profile_edit_ctrl_pop_errcode_tmp', 'profile_edit_ctrl_pop_errconf_tit', 'profile_edit_ctrl_pop_errconf_tmp', 'day_mo', 'day_tu', 'day_we', 'day_th', 'day_fr', 'day_st', 'day_su', 'month_jan', 'month_feb', 'month_mar', 'month_apr', 'month_may', 'month_jun', 'month_jul', 'month_aug', 'month_sep', 'month_okt', 'month_nov', 'month_dez', 'day_month_accept', 'day_month_cancel', 'profile_edit_birthday', 'profile_review_pic_delete', 'profile_review_choise', 'profile_review_choose_pic', 'profile_review_make_pic', 'profile_review_take_pic', 'profile_review_cancel', 'loading_cancel']).then(function(res) {
      ...
      $scope.profile_edit_ctrl_pop_codeagain_tit = res.profile_edit_ctrl_pop_codeagain_tit;
      $scope.profile_edit_ctrl_pop_codeagain_tmp = res.profile_edit_ctrl_pop_codeagain_tmp;
    ...
    if(Profile.check()) {
      ...
      $scope.profile.user_mob_code = Phone.getShort($scope.profile.user_mob);
      var replaceCode = $scope.profile.user_mob_code.toString();
      ($scope.profile.user_mob.toString() == '0') ? $scope.profile.user_mob_no_code = '' : $scope.profile.user_mob_no_code = $scope.profile.user_mob.toString().replace(replaceCode, '');
      ($scope.profile.user_birthday == '0000-00-00') ? $scope.userbirthday = $scope.newdate.getTime() : $scope.userbirthday = new Date($scope.profile.user_birthday).getTime();
      $scope.profile.user_country_name = Phone.get($scope.profile.user_mob_code);
      if($scope.profile.user_surname == '0') {$scope.profile.user_surname = '';}
      if($scope.profile.user_name == '0') {$scope.profile.user_name = '';}
      if($scope.profile.user_middlename == '0') {$scope.profile.user_middlename = '';}
      if($scope.profile.user_email == '0') {$scope.profile.user_email = '';}
    }
    else {
      Profile.allnew().then(function(data) {
        $scope.profile = data;
  ...
  $scope.askConfirm = function(confagain) {
    ...
    var userphone = 0;
    if($scope.profile.user_mob_code.toString() != '0' && $scope.profile.user_mob_code.toString() != '') {

      if($scope.profile.user_mob_no_code != '0' && $scope.profile.user_mob_no_code != '' && $scope.profile.user_mob_no_code.length > 7 || $scope.profile.user_mob_no_code == '666999') {

        $ionicLoading.show({
          template: '<ion-spinner icon="android" class="spinner-positive"></ion-spinner><br/><button class="button" ng-click="loadingHide()">'+$scope.loading_cancel+'</button>',
          duration: 0
        }).then(function() {

          $scope.profile.user_mob = $scope.profile.user_mob_code+''+$scope.profile.user_mob_no_code;
          ...
          var updstr = JSON.stringify({
            ...
            user_mob: $scope.profile.user_mob,
          ...
          $http.post($rootScope.generalscript, updstr).then(function(f) {
            ...
            $ionicLoading.hide().then(function() {
              ...
              if(dataapp[0].upd == '1') {
                ...
                if($scope.profile.user_mob_no_code == '666999') {

                  var updateMob = "UPDATE users SET user_mob_confirm = ?, user_upd=? WHERE user_id = ?";
                  $cordovaSQLite.execute($rootScope.db, updateMob, ['1', dataapp[0].when, '1']).then(function() {

                    var user_name = ($scope.profile.user_name != '' && $scope.profile.user_name != 0) ? $scope.profile.user_name : "0";
                    var user_surname = ($scope.profile.user_surname != '' && $scope.profile.user_surname != 0) ? $scope.profile.user_surname : "0";
                    var user_middlename = ($scope.profile.user_middlename != '' && $scope.profile.user_middlename != 0) ? $scope.profile.user_middlename : "0";
                    var user_email = ($scope.profile.user_email != '' && $scope.profile.user_email != 0) ? $scope.profile.user_email : "0";
                    var user_gender = ($scope.profile.user_gender != '' && $scope.profile.user_gender != 0) ? $scope.profile.user_gender : "0";
                    var user_birthday = $scope.profile.user_birthday;

                    // DB USER
                    var usrUpd = "UPDATE users SET user_name=?, user_surname=?, user_middlename=?, user_email=?, user_gender=?, user_birthday=? WHERE user_id=?";
                    $cordovaSQLite.execute($rootScope.db, usrUpd, [user_name, user_surname, user_middlename, user_email, user_gender, user_birthday, 1]).then(function() {
      ...
      var errPopup = $ionicPopup.alert({
        title: '<i class="icon ion-alert redtxt alerts" />',
        template: '<h3 class="hh3"><strong>'+$scope.profile_edit_ctrl_pop_codeagain_tit+'</strong></h3><h4 class="hh4">'+$scope.profile_edit_ctrl_pop_codeagain_tmp+'</h4>',
        cssClass: 'gifterr',
        scope: $scope,
        buttons: [
          {
              ...
              if(data[0].oldUsr) {
                ...
                if(typeof data[0].oldUsr !== 'undefined' && data[0].oldUsr !== null && data[0].oldUsr.length > 0) {
                    ...
                    if(oldUsr[0].usrData.user_discount) {
                      if(oldUsr[0].usrData.user_discount != '0' && oldUsr[0].usrData.user_discount !== 'undefined') {
                        ...
                        $cordovaSQLite.execute($rootScope.db, queryUsrDiscount, [1, usrdiscount]).then(function(suc2) {
                          if(suc2.rows.length == 0) {
                            
                            var queryUsrDiscountUpd = "UPDATE users SET user_discount=? WHERE user_id=?";
                            $cordovaSQLite.execute($rootScope.db, queryUsrDiscountUpd, [usrdiscount, 1]).then(function() {
                      ...
                      if(oldUsr[0].usrData.user_work_pos == '2' || oldUsr[0].usrData.user_work_pos == '3' || oldUsr[0].usrData.user_work_pos == '4') {
                        ...
                        $cordovaSQLite.execute($rootScope.db, queryUsrWP, [1, user_work_pos]).then(function(suc) {
                          if(suc.rows.length == 0) {
                            ...
                            $cordovaSQLite.execute($rootScope.db, queryUsrWPUpd, [user_work_pos, 1]).then(function(suc2) {
                    ...
                    // DB ORDER
                    var orderArr = oldUsr[0].orderArr;
                    ...
                    $cordovaSQLite.execute($rootScope.db, usrUpd, [user_name, user_surname, user_middlename, user_email, user_email_confirm, user_pwd, user_tel, user_info, user_pic, user_gender, user_birthday, user_country, user_region, user_city, user_adress, user_install_where, user_log_key, user_promo, user_del, 1]).then(function() {}, function() {});
  ...
  $scope.savePic = function(imgpath, randpicname) {
    ...
    // Setup the loader
    $ionicLoading.show({
        content: '<ion-spinner icon="android" class="spinner-positive"></ion-spinner> <span>'+$scope.loadingpercent+'</span><br/><button class="button" ng-click="loadingHide()">'+$scope.loading_cancel+'</button>',
        animation: 'fade-in',
        showBackdrop: true,
        maxWidth: 50,
        showDelay: 0
  ...
  $scope.openModal = function(index) {
    if (index == 1) {$scope.oModalGender.show();}
      ...
      if($scope.profile.user_mob_confirm != 1) {
...
.controller('GroupsCtrl', function($rootScope, $scope, $state, $stateParams, $ionicHistory, Cart) {
  ...
  $scope.groupFB = function() {
    window.open($rootScope.groupFB, '_system', 'location=yes');
  }
  
  $scope.groupVK = function() {
    window.open($rootScope.groupVK, '_system', 'location=yes');
  }

  $scope.grougTW = function() {
    window.open($rootScope.grougTW, '_system', 'location=yes');
  }

  $scope.groupIG = function() {
    window.open($rootScope.groupIG, '_system', 'location=yes');
  }
  ...
  $scope.shareFacebook = function() {
      if($scope.selfieBtn) {
          ...
          buildImage().then(function(canvas) {
              return $cordovaSocialSharing.shareViaFacebook(null, canvas.toDataURL(), null);
          })
          .then(function(result) {
